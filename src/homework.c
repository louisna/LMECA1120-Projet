...
void femCouetteSolve(femCouetteProblem *theProblem){
    ...
    //des que la solution a convergee, on arrete de resoudre
...
void findElement(femCouetteProblem *theProblem){
    ...
    int i,j,k;
    double x_elem[3], y_elem[3], iso[2];
    int map[3];
...
void femCouetteAssemble(femCouetteProblem *theProblem)
{
    ...
    /* Influence du fluide sur les billes*/
        ...
        for (j=0; j < 3; j++)
        {
            ...
            for (k = 0; k < 3; k++)
            {
              for(l = 0; l < 3; l++)
              {
                  ...
                  theSystem->A[map[k]][map[l]]  += mu*numIntegrateA;
                  theSystem2->A[map[k]][map[l]] += mu*numIntegrateA;
              }
              theSystem->B[map[k]]  = 0;
